 ...
 * clause-numbered deep anchors (/rules#6.4) and LIVE demo boards powered by
    ...
    <main className="compendium min-h-screen">
      <div className="mx-auto max-w-3xl px-6 pb-20">
        ...
        <Section id="2" title="§2 · The board">
          ...
          <DemoBoard
            state={wrapDemo}
            caption="§5.6 — tap the rook: file-wise moves wrap straight through the 32↔1 seam."
            className="mx-auto max-w-sm"
          />
        </Section>

        {/* ---- 4 ---- */}
        <Section id="4" title="§4 · Turns, and the double-move opening">
          <p>
            ...
            meridian, recorded as a single turn (§4.2). The table built this
        ...
        {/* ---- 5 ---- */}
        <Section id="5" title="§5 · How the pieces move">
          <p>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <DemoBoard
              state={pawnDemo}
              caption="§5.7 — tap each pawn: they advance AWAY from their own meridian, in opposite directions."
            />
            <DemoBoard
              state={curlDemo}
              caption="§5.4 — tap the bishop: the banana curl, with one rail bounce per move."
            />
          </div>
          <p id="5.4">
          ...
          <DemoBoard
            state={promoDemo}
            caption="§5.7/§8.3 — tap the pawn: one step from the enemy back rank, four promotion choices."
            className="mx-auto max-w-sm"
          />
        </Section>

        {/* ---- 6 ---- */}
        <Section id="6" title="§6 · The meridian, the halo, and the Avenger">
          <p id="6.2">
          ...
          <p id="6.3">
            ...
            (§6.3). The app will warn you and let you do it anyway; after 250
            games, the kamikaze capture is a known weapon.
          </p>
          <p id="6.4">
            The <strong>Avenger</strong> exemption (§6.4): a primary piece
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <DemoBoard
              state={evapDemo}
              caption="§6.3 — tap the unhaloed knight: crossing moves complete, then the piece evaporates."
            />
            <DemoBoard
              state={haloDemo}
              caption="§6.2 — the same knight, haloed: home is open, forever."
            />
          </div>
        </Section>

        {/* ---- 7 ---- */}
        <Section id="7" title="§7 · Check, and when mate actually lands">
          <p id="7.3">
            You may never leave your own king in check — even certain your
            partner would fix it (§7.1). You owe your partner's king nothing
            (§7.2). And the rule that surprises every chess player:{" "}
            <strong>
              it is not checkmate until the threatened player's own turn
              arrives with no legal escape (§7.3)
            </strong>
        ...
        {/* ---- 8 ---- */}
        <Section id="8" title="§8 · The special rules">
          <p id="8.1">
            <strong>En passant (§8.1)</strong> works as in standard chess,
            available on the move immediately following the double step.
          </p>
          <p id="8.2">
            <strong>Castling (§8.2)</strong> comes in two shapes. Queenside:
          ...
          <DemoBoard
            state={castleDemo}
            caption="§8.2 — tap the king: both castling shapes, live. (O-O-O swaps K and Q; O-O runs the king inward.)"
            className="mx-auto max-w-sm"
          />
          <p id="8.4">
            <strong>Draws.</strong> Stalemate is a draw for all four (§8.4).
            ...
            (§8.5–8.6). All four players may also simply agree (§8.7).
          </p>
        </Section>
...
function Section({
  ...
  return (
    <section id={id} className="scroll-mt-8 space-y-4 pt-10">
      <h2
